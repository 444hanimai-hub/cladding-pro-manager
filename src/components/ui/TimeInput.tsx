  ...
  return (
    <div className={cn("relative group", className)}>
      <input
        ...
        className={cn(
          "w-full border-none rounded-xl px-4 py-2 transition-all font-medium pr-10 bg-[#F5F5F0] text-[#141414] focus:ring-[#5A5A40]/30",
          "text-xs"
        )}
      />
      <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
        <div className="relative">
          <Clock size={14} className={cn("transition-colors cursor-pointer", 
            "text-[#141414]/20 group-hover:text-[#5A5A40]")} />
          <input 
            type="time"
            value={value || ''}
            onChange={(e) => {
