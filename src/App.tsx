...
export default function App() {
  ...
  useEffect(() => {
    // On mount, if mobile, open sidebar as requested: "должна открываться сразу при входе"
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        // Find user in database either by UID or Email
        ...
        if (!querySnap.empty) {
          ...
          // New user (or owner) - anyone can join now
          await setDoc(doc(db, 'users', user.uid), {
            uid: user.uid,
            email: user.email || '',
